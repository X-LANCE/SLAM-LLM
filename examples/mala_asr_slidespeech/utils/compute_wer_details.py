...
def get_args():
    ...
    parser.add_argument("--ref2session", type=str, default="", help="Text input path")
...
def main(args):
    ...
    if split and not case_sensitive: #x
        newsplit = dict()
        for w in split:
            words = split[w]
    ...
    multirec_session_result = None
    if args.ref2session != "": multirec_session_result = dict()
    for i, hyp_file in enumerate(rec_files):
        ...
        multirec_session_result[rec_names[i]] = dict()
    ...
    for line in tqdm(open(ref_file, 'r', encoding='utf-8'), total=_file_total_len):
        ...
        for rec_name in rec_names:
            ...
            if utt2session is not None:
                session_result = multirec_session_result[rec_name]
                session = utt2session[fid]
                if session not in session_result:
                    session_result[session] = {'lab': [], 'rec': [], 'all': 0, 'cor': 0, 'sub': 0, 'ins': 0, 'del': 0}

                session_result[session]['all'] = result['all'] + session_result[session]['all']
                session_result[session]['cor'] = result['cor'] + session_result[session]['cor']
                session_result[session]['sub'] = result['sub'] + session_result[session]['sub']
                session_result[session]['ins'] = result['ins'] + session_result[session]['ins']
                session_result[session]['del'] = result['del'] + session_result[session]['del']
            ...
            _tp = _tn = _fp = _fn = 0
            hot_true_list = set([hotword for hotword in ocr_text if hotword in _label_list])
            hot_bad_list = set([hotword for hotword in ocr_text if hotword not in _label_list])
    ...
    # print(zip(multirec_session_result))
    session_results_zip = dict()
    for rec_name, session_result in multirec_session_result.items():
        for session, result in session_result.items():
            if session not in session_results_zip.keys():
                session_results_zip[session] = []
            session_results_zip[session].append((rec_name, result))
            # if result['all'] != 0 :
            #     wer = float(result['ins'] + result['sub'] + result['del']) * 100.0 / result['all']
            # else :
            #     wer = 0.0
            # print('{} {} -> {:4.2f} %'.format(rec_name, session, wer), end = ' ')
            # print('N=%d C=%d S=%d D=%d I=%d' %
            #     (result['all'], result['cor'], result['sub'], result['del'], result['ins']))
    for session, result_list in session_results_zip.items():
        for rec_name, result in result_list:
            if result['all'] != 0:
                wer = float(result['ins'] + result['sub'] + result['del']) * 100.0 / result['all']
            else:
                wer = 0.0
    ...
    for rec_name in rec_names:
        ...
        recall =  hotwords_related_dict[rec_name]['tp'] / (
        ...
        print()
        print("{:.3f}; {:.3f}; {:.3f}; {:.2f}%".format(ub_wer_dict[rec_name]['wer'].get_wer(), ub_wer_dict[rec_name]['u_wer'].get_wer(), ub_wer_dict[rec_name]['b_wer'].get_wer(), recall))
