...
class MultiTaskDataset(Dataset):
    def __init__(self, dataset_config, tokenizer=None, split='train',musan_path=None):
        ...
        self.prev_ark_file = None
        self.prev_ark_fd = None
        self.read_time = 0
        self.open_time = 0
        self.total_time = 0
    ...
    def kaldiio(self,ark_path):
        start_time = time.time()
        ark_file, offset, slices = _parse_arkpath(ark_path)
        if self.prev_ark_file == ark_file:
            ark_fd = self.prev_ark_fd
        else:
            if self.prev_ark_fd is not None:
                self.prev_ark_fd.close()
            ark_fd = open_like_kaldi(ark_file, 'rb')
        self.open_time += time.time() - start_time
        start_time = time.time()
        sample_rate, waveform = _load_mat(ark_fd, offset, slices, endian='<')
        waveform = waveform.astype(np.float32) / (1 << 15)
        self.read_time += time.time() - start_time
        # if waveform.ndim == 2:
        #     waveform = waveform.T
        # else:
        #     waveform = np.expand_dims(waveform, axis=0)
        self.prev_ark_file, self.prev_ark_fd = ark_file, ark_fd
        return waveform
    ...
    def __getitem__(self, index):
        if self.last_epoch_time != 0:
            self.total_time += time.time() - self.last_epoch_time
        self.last_epoch_time = time.time()
        if (index + 1) % 100 == 0:
            print(f"[open_time] {self.open_time} [read_time] {self.read_time} [total_time] {self.total_time} ")
        ...
        audio_raw = self.kaldiio(ark_path)
