...
ckpt_path=/nfs/yangguanrou.ygr/codes/SLAM-LLM/examples/s2s/scripts/ygr/exp/tts/premium_pretrain_remake_cosyvoice2_codeclayer2/s2s_epoch_6_step_5955
split=test
...
audio_specialtokens=39
padded_audio_vocabsize=$((audio_vocabsize + audio_specialtokens))
eoa=$audio_vocabsize 
pad_a=$((audio_vocabsize+1))
input_a=$((audio_vocabsize+2))
answer_a=$((audio_vocabsize+3))
split=$((audio_vocabsize+4))
total_audio_vocabsize=6600          # the vocab size of the codec token
...
speech_sample_rate=24000 #只对算时间有用
...
python $code_dir/inference_s2s.py \
        ...
        ++model_config.vocab_config.audio_specialtokens=$audio_specialtokens \
        ++model_config.vocab_config.padded_audio_vocabsize=$padded_audio_vocabsize \
        ++model_config.vocab_config.eoa=$eoa \
        ++model_config.vocab_config.pad_a=$pad_a \
        ++model_config.vocab_config.input_a=$input_a \
        ++model_config.vocab_config.answer_a=$answer_a \
        ++model_config.vocab_config.split=$split \
        ...
        ++model_config.cosyvoice_version=$cosyvoice_version \
        ...
        ++dataset_config.vocab_config.audio_specialtokens=$audio_specialtokens \
        ++dataset_config.vocab_config.padded_audio_vocabsize=$padded_audio_vocabsize \
        ++dataset_config.vocab_config.eoa=$eoa \
        ++dataset_config.vocab_config.pad_a=$pad_a \
        ++dataset_config.vocab_config.input_a=$input_a \
        ++dataset_config.vocab_config.answer_a=$answer_a \
        ++dataset_config.vocab_config.split=$split \
...
#python examples/s2s/utils/eval_emo.py --gt /nfs/yangguanrou.ygr/data/secap_my/test.jsonl --pred