...
import sys
import copy
sys.path.append('/aistor/aispeech/hpc_stor01/home/fangyangui/workingspace/project/SLAM-LLM/examples/asr_fireredasr_text/model')
...
def load_firered_llm_model_and_tokenizer(model_path, encoder_path, llm_dir, train_config):
    ...
    # input()
    package["args"].use_lora = True
    # package["args"].freeze_llm = True
...
class FireRedDatasetLarge(IterableDataset):
    ...
    #     keys = [sample["key"] for sample in samples]
    #     targets = [sample["target"] for sample in samples]
    #     result = self.tokenizer(targets,return_tensors="pt",padding=True)
    #     if self.inference_mode:
    #         result["keys"] = keys
    #         result["targets"] = targets
    #     else:
    #         result["labels"] = copy.deepcopy(result["input_ids"])
    #         # result["labels"][:,:len(result["labels"])]
    #     return result
