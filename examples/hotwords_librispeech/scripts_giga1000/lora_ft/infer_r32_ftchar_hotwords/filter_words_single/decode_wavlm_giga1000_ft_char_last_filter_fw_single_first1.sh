...
for ref_split in test_name; do
        decode_log=$ckpt_path/decode_${ref_split}_beam4_filter_fw_single_first${first}
        python $code_dir/inference_asr_batch.py \
                ...
                ++dataset_config.file=src/slam_llm/datasets/giga_hotwordsinfer_single_dataset.py:get_speech_dataset \
                ++dataset_config.ctc_file=/nfs/yangguanrou.ygr/data/gigaspeech_my_infer/wavlm_ft_char_giga1000.txt \
                ++dataset_config.word_num=5 \
                ++dataset_config.probability_threshold=0.95 \
                ++dataset_config.filter_infer_sentence=true \
                ++dataset_config.filter_infer_sentence_few=true \
                ++dataset_config.first=${first} \
...
# bash examples/hotwords_librispeech/scripts_giga1000/lora_ft/infer_r32_ftchar_hotwords/filter_words_single/decode_wavlm_giga1000_ft_char_last_filter_fw_single_first1.sh > examples/hotwords_librispeech/scripts_giga1000/lora_ft/infer_r32_ftchar_hotwords/filter_words_single/decode_wavlm_giga1000_ft_char_last_filter_fw_single_first1.log