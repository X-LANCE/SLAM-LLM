...
class GigaDataset(Dataset):
        ...
        if split == "train":
            ...
            with open(dataset_config.dev_scp_file_path + "2/giga_ner_phone_wsplit.txt",'r') as f:
                for line in f:
                    ...
                    label_phoneme_list=line[4].replace("SPN","SPN SPN")
                    self.label_phoneme_list.append(label_phoneme_list)
            ...
            with open(dataset_config.dev_scp_file_path + "person_uniq_my_phone_spn",'r') as f:  #spn也去重过的
                for line in f:
                    line = line.strip()
                    line = line.replace("SPN SPN SPN","SPN SPN")
                    self.name_phoneme_list.append(line)
            self.ngram_index = build_ngram_index(self.name_phoneme_list)

            
            self.phn_to_name_dict["SPN SPN"]=[]
            with open(dataset_config.dev_scp_file_path + "person_uniq_my",'r') as ftext:
                with open(dataset_config.dev_scp_file_path + "person_uniq_my_phone") as fp:
                    for text,p in zip(ftext,fp):
                        text = text.strip()
                        p = p.strip()
                        p = p.replace("SPN","SPN SPN")
                        self.name_to_phn_dict[text]=p

                        # if p != "SPN SPN":
                        #     self.phn_to_name_dict[p]=text
                        # else:
                        #     self.phn_to_name_dict["SPN SPN"].append(text)
                        if p in self.phn_to_name_dict:
                            self.phn_to_name_dict[p].append(text)
                        else:
                            self.phn_to_name_dict[p]=[text]
    ...
    def __getitem__(self, index):
        ...
        # line_name = self.line_name_list[index]  #'KIM' #??
        # ocr = self.name_list[index] #'PETER' # upperbound
        # ocr = " ".join(self.name_list)#'PETER'  #debug 
        ...
        #     if value > 0.8:
        #         filtered_dict.append((k, value))
        #         if len(filtered_dict) == 20:
        #             break  # 如果已经找到20个符合条件的元组，则停止循环
        # keys_list = [k for k, _ in filtered_dict]
