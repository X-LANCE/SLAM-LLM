...
class AVSRDataset(Dataset):
    ...
    def __getitem__(self, index):  #avsr 是shuffle的dataloader echat好像默认false 没shu  index从0开始
        ...
        if index==99639 or index== 71740 or index==19753 or index==14116 or index==49729 or index==26726 or index==58675:  #dirty data
            index+=1
        ...
        if index < 118516:     #原本是96318   查过了 这个数确实是lrs2的那个行数 也就是文件数  原本应该是pretrain处理的 有一部分搞到main处理了 所以没有crop 导致超过500
            ...
            while inp==0 and trgtin ==0 and  trgtout ==0 and trgtLen==0 and  target ==0:
                index+=1
                targetFile = self.datalist[index] + ".txt"
        ...
        if self.model_config.modal=="AV":
            ...
            prompt = self.dataset_config.prompt #"Transcribe video to text. " #"Transcribe the silent speech in this video to text by lip-reading the speaker's clear and visible lip movements."
        ...
        if self.model_config.encoder_name == "moco_wav2vec2":
            audio_length = self.calculate_output_length(audio_raw,visual_raw)  #video  #tensor(80)
        elif self.model_config.encoder_name == "sota_avsr" or self.model_config.encoder_name =="av_hubert":
            audio_length = visual_raw.shape[0]
        elif self.model_config.encoder_name == "whisper":
            audio_length = (audio_mel.shape[0] + 1) // 2  #1500
    ...
    def calculate_output_length(self,audio_raw,visual_raw):
        ...
        if self.modal == "AO":
            ...
            vidPadding = torch.zeros(1).long().to(vidLen.device)
            ...
            vidLeftPadding = torch.floor(torch.div(vidPadding, 2)).int()  #tensor(7, dtype=torch.int32)
            vidRightPadding = torch.ceil(torch.div(vidPadding, 2)).int() #tensor(8, dtype=torch.int32)
            audLeftPadding = torch.floor(torch.div(audPadding, 2)).int()  
            audRightPadding = torch.ceil(torch.div(audPadding, 2)).int()

            audioBatch = torch.randn(audLen,1024) #.to('cuda') # pseudo audio Batch
            videoBatch = torch.randn(vidLen,2048) #.to('cuda') # pseudo audio Batch   #torch.Size([65, 2048])
            ...
            inputLenBatch = (vidLen + vidPadding).long() #tensor(87)

        if self.modal=="A_V":
            inputLenBatch = inputLenBatch*2 #tensor(174)
        #  过卷积层  其实没有变 就用inputLen
        return inputLenBatch
    ...
    def collator(self, samples):
        ...
        if not self.modal == "AO":
            ...
            vis_padding_mask = torch.zeros((len(vis_seq_list), len(max(vis_seq_list, key=len))), dtype=torch.bool)
            for i, seq in enumerate(vis_seq_list):
                vis_padding_mask[i, vis_len[i]:] = True
        else:
            vis_seq_list = None
            vis_len = None
            vis_padding_mask = None
        ...
        return {
            ...
            'audio_mel': audio_mel, #only for whisper
            'audio_mel_post_mask': audio_mel_post_mask,  #only for whisper
            'modality_mask': modality_mask,
