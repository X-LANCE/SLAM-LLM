...
import llama_recipes.utils.custom_utils as custom_utils
    ...
    with open(manifest_path) as f:
        ...
        for ind, line in enumerate(f):
            ...
            if min_keep is not None and sz < min_keep:
                ...
                video_path = items[1]  # '/nfs/yangguanrou.ygr/LRS_new/video/test/0Fi83BHQsMA/00002.mp4'
                audio_path = items[2]  # '/nfs/yangguanrou.ygr/LRS_new/audio/test/0Fi83BHQsMA/00002.wav'
                audio_id = items[0]
...
class AVHubertdataset(torch.utils.data.Dataset):
    def __init__(self, dataset_config, model_config, tokenizer=None, split='train', ):
        ...
        # AVHUBERT
        self.data = "/nfs/yangguanrou.ygr/LRS_new/433h_data"
        self.manifest = f"{self.data}/{split}.tsv"  # '/nfs/yangguanrou.ygr/LRS_new/433h_data//test.tsv'

        # paths = [
        #     f"{self.get_label_dir()}/{split}.{l}" for l in dataset_config.labels
        # ]
        paths = [
        ...
        image_aug = False
        noise_fn, noise_snr = f"{self.cfg.noise_wav}/{split}.tsv" if dataset_config.noise_wav is not None else None, eval(
            dataset_config.noise_snr)
        ...
        self.audio_root, self.names, inds, tot, self.sizes = load_audio_visual(self.manifest,
                                                                               dataset_config.max_sample_size,
                                                                               dataset_config.min_sample_size,
                                                                               frame_rate=dataset_config.sample_rate,
        ...
        self.noise_wav, self.noise_prob, self.noise_snr, self.noise_num = [ln.strip() for ln in open(
    ...
    def get_label(self, index, label_idx):
        ...
        #     label = self.label_processors[label_idx](label)   #会把字符串变成数字
        return label
    ...
    def __getitem__(self, index):
        ...
        audio_feats, video_feats = torch.from_numpy(
            audio_feats.astype(np.float32)) if audio_feats is not None else None, torch.from_numpy(
            video_feats.astype(np.float32)) if video_feats is not None else None
        if self.normalize and 'audio' in self.modalities:  # torch.Size([110, 88, 88, 1]),None  x
            with torch.no_grad():
    ...
    def collator(self, samples):
        ...
        input_ids = torch.stack(
            [self.pad(s['input_ids'], input_ids_max_length, self.tokenizer.pad_token_id) for s in samples])
