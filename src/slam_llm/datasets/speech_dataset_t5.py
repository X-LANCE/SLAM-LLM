...
class SpeechDatasetJsonl(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        prompt_ids = self.tokenizer.encode(prompt)[:-1] # remove eos
        ...
        if self.inference_mode:
            example_mask = torch.ones((prompt_length+audio_length,), dtype=bool)
        ...
        # prompt_ids = torch.cat((prompt_ids, audio_pseudo))  # [prompt,audio]

        labels_ids = self.tokenizer.encode(answer)  # [answer]
        # labels_ids.append(self.tokenizer.eos_token_id)  # [answer,eos]
        labels_ids = torch.tensor(
            labels_ids, dtype=torch.int64
        )
        # labels_ids = copy.deepcopy(example_ids)  # [audio,prompt,answer,eos]
        # labels_ids[:audio_length + prompt_length] = -1  # [-1,-1,answer,eos];
        example_mask = torch.ones((prompt_length+audio_length,), dtype=bool)
        # example_mask = prompt_ids.ge(-1)  # FIX(GZF): [True,True,True,True]

        # label_mask = labels_ids.ge(0)  # [False,False,True,True]
        # prompt_ids[~example_mask] = 0  # [audio,prompt,answer,eos]
    ...
    def collator(self, samples):
        ...
        input_ids = torch.stack([self.pad(s['input_ids'], input_ids_max_length, self.tokenizer.pad_token_id)
                                 for s in samples])
        attention_mask_max_length = max([s['attention_mask'].shape[0] for s in samples])
        attention_mask = torch.stack([self.pad(s['attention_mask'], attention_mask_max_length, False)
                                      for s in samples])
        if self.input_type == "raw":
        ...
        prompt_lengths = torch.tensor([sample['prompt_length'] for sample in samples], dtype=torch.int64)
        audio_lengths = torch.tensor([sample['audio_length'] for sample in samples], dtype=torch.int64)
        if self.inference_mode:
        ...
        labels_max_length = max([s['labels'].shape[0] for s in samples])
        labels = torch.stack([self.pad(s['labels'], labels_max_length, self.IGNORE_INDEX)
                              for s in samples])
        return {
            "input_ids": input_ids,
            "labels": labels,
