...
def train_dpo(model, ref_model, train_dataloader,eval_dataloader, tokenizer, optimizer, lr_scheduler, gradient_accumulation_steps, train_config, log_config, fsdp_config=None, local_rank=None, rank=None):
    """
    ...
    for epoch in range(train_config.num_epochs):
        ...
        with MemoryTrace() as memtrace:  # track the memory usage
            ...
            for step, batch in enumerate(train_dataloader):
                for kkey, vvalue in batch.items():
                    for key in vvalue.keys():
                        if train_config.enable_fsdp or train_config.enable_ddp:
                            batch[kkey][key] = batch[kkey][key].to(local_rank) if isinstance(batch[kkey][key], torch.Tensor) else batch[kkey][key]
                            if isinstance(batch[kkey][key], dict):
                                for k2 in batch[kkey][key].keys():
                                    batch[kkey][key][k2] = batch[kkey][key][k2].to(local_rank) if isinstance(batch[kkey][key][k2], torch.Tensor) else batch[kkey][key][k2]
                        else:
                            batch[kkey][key] = batch[kkey][key].to('cuda:0') if isinstance(batch[kkey][key], torch.Tensor) else batch[kkey][key]
                            if isinstance(batch[kkey][key], dict):
                                for k2 in batch[kkey][key].keys():
                                    batch[kkey][key][k2] = batch[kkey][key][k2].to('cuda:0') if isinstance(batch[kkey][key][k2], torch.Tensor) else batch[kkey][key][k2]
                
                with autocast(): #精度而已：会让框架来决定哪些操作用 float16 / bfloat16，哪些依然需要保持在更高精度（float32）来保证数值正确性。
                    # 要推理四次，看下除了loss还需要记什么指标：forward里把正例和负例都算一下
                    result_dict = model(**batch)
                    chosen_logps, reject_logps, chosen_text_acc, chosen_audio_acc, chosen_loss_recorder = (result_dict[key] for key in ["chosen_logps", "reject_logps", "chosen_text_acc", "chosen_audio_acc", "chosen_loss_recorder"])
                    
                    ref_batch = copy.deepcopy(batch)      # 做一个深拷贝
                    with torch.no_grad():
                        # ref_result_dict = ref_model(**batch)
                        ref_result_dict = ref_model(**ref_batch)
                    ref_chosen_logps, ref_reject_logps, ref_chosen_text_acc, ref_chosen_audio_acc, ref_chosen_loss_recorder = (ref_result_dict[key] for key in ["chosen_logps", "reject_logps", "chosen_text_acc", "chosen_audio_acc", "chosen_loss_recorder"])

                    # 计算 DPO LOSS
                    # pdb.set_trace()
                    logratios = chosen_logps - reject_logps
                    ref_logratios = ref_chosen_logps - ref_reject_logps
                    logratios = logratios.to(local_rank)
                    ref_logratios = ref_logratios.to(local_rank)
                    logits = logratios - ref_logratios
                    losses = -F.logsigmoid(train_config.beta * logits)

                    # losses = -F.logsigmoid(self.beta * logits) * (1 - self.label_smoothing) - F.logsigmoid(-self.beta * logits) * self.label_smoothing  #默认self.label_smoothing=0
                    chosen_rewards = train_config.beta * (chosen_logps.to(local_rank) - ref_chosen_logps.to(local_rank)).detach()
                    reject_rewards = train_config.beta * (reject_logps.to(local_rank) - ref_reject_logps.to(local_rank)).detach()


                acc = chosen_text_acc #text_acc
                audio_acc = chosen_audio_acc
                if train_config.modeling_paradigm == "parallel":
                    layer_loss = chosen_loss_recorder
                else:
                ...
                if (epoch * total_length + step + 1) % train_config.validation_interval == 0 and train_config.run_validation:
                    eval_ppl, eval_epoch_loss, *rest = evaluation_dpo(model, ref_model, train_config, eval_dataloader, local_rank, tokenizer)
                    eval_epoch_acc = rest[0] if rest else -1
                    eval_epoch_audio_acc = rest[1] if rest else -1
                    checkpoint_start_time = time.perf_counter()
                    # if train_config.save_model and (eval_epoch_loss < best_val_loss): #改这一行
                    ...
                    if train_config.save_model:
                        ...
                        if train_config.enable_fsdp or train_config.enable_ddp: #x
                            dist.barrier()
...
def evaluation_dpo(model, ref_model, train_config, eval_dataloader, local_rank, tokenizer):
    """
    ...
    with MemoryTrace() as memtrace:
        ...
        for step, batch in enumerate(eval_dataloader):
            ...
            # try:
            # preds = torch.argmax(chosen_model_outputs_logits, -1)
            # eval_preds.extend(
