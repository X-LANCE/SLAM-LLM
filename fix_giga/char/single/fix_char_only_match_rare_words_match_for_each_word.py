...
log_filename = "fix_giga/char/single/fix_char_{}_only_match_rare_words_match_for_each_word_5000_remove_all_common.log".format(first)
prompt_word_num=0
...
with open("/nfs/yangguanrou.ygr/data/ner/giga_name_test/person_uniq_my_tn_single",'r') as f:
    for line in f:
        line = line.strip()
        if line not in common_words_5k:
...
name_list=list(set(name_list)) #1435
...
for index in tqdm(range(len(data_list))):
    ...
    # filtered_words = list(dict.fromkeys(filtered_words))  效果不好
